...
void pathCount_rec(struct Graph* G,int s,int d,int* visited,int* count,int* path,int *index){
    visited[s] = 1;
    path[*index] = s;
    (*index)++;
    if(s == d){
        ...
        struct Node* temp = NULL;
            ...
            for(temp = G->array[s].head;temp!=NULL;temp=temp->next){
                if(visited[temp->data] != 1){
                    pathCount_rec(G,temp->data,d,visited,count,path,index);
                }
            }
    }
...
int pathCount(struct Graph* G,int s,int d){
    int* visited = (int *)malloc(G->V * sizeof(int));
    ...
    pathCount_rec(G,s,d,visited,&count,path,&index);
...
int main(){
    ...
    //kosaraju(G);
    printf("\nPath count from 0 to 2 is %d",pathCount(G,0,2));
